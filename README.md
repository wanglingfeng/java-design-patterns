...
#### 防止单例模式被反射攻击
* 构造器在被要求创建第二个实例时抛出异常(SyncSingleton)
* 用枚举类写单例模式(EnumSingleton)
...
#### 参与者
* 产品(SoccerTeam) - 被构造的复杂对象
* 抽象建造者(SoccerTeamBuilder) - 一个抽象接口来规范产品对象的各个部件的建造
* 具体建造者(RealMadridBuilder) - 抽象建造者的实现类，构造产品和装配产品的各个部件
* 导演者(SoccerTeamDirector) - 调用具体建造者来创建产品对象
...
### 外观模式(Facade pattern)
外观模式是为了解决类与类之间的依赖关系，将他们的关系放在一个Facade类`Computer类`中，降低了类与类之间的耦合度。
